package github_test

import (
	"context"
	"testing"
	"time"

	"github.com/coreos/go-oidc/v3/oidc"
	"github.com/stretchr/testify/assert"
	"github.com/stretchr/testify/require"
	"github.com/thepwagner/github-token-action-server/github"
)

// This token is expired
const tok = `eyJ0eXAiOiJKV1QiLCJhbGciOiJSUzI1NiIsIng1dCI6ImVCWl9jbjNzWFlBZDBjaDRUSEJLSElnT3dPRSIsImtpZCI6Ijc4MTY3RjcyN0RFQzVEODAxREQxQzg3ODRDNzA0QTFDODgwRUMwRTEifQ..1zr6A6Hc2sTZLMHRvJd925lG27ou0bAQoNkUnLY9RrRn-p7xSzqwlhm1wZxyOjILHSQDpgPy0COc2kQxCQPIusH2vxvuAOW7jrVphaKKC8aSamLe1Rs3VTec1u6bP_JDO2BWZuIcrnFd-yBlbyytXVlRw26Q6ZjkY7EoQXnHEmlskArcuFlTjjksaRRk2TPSnMyKBfWvrTNtVs1mN2CjzeX104NZ5qk-3m2jTInNTI-XJ7ZPlXsa0V12AzOzd9NdkdUe62Qk5LBN4W3XSUZWhS-nK3LjraRgKk_6JS8tFPKglsxUyYTpP3N8VPvYWRnjMzwRaJn1FagjwhSQ8K4NsQ`

var tokWasValid, _ = time.Parse(time.RFC3339, "2022-07-15T11:26:30Z")

func TestNewTokenParser_Parse(t *testing.T) {
	ctx := context.Background()
	tp, err := github.NewOIDCParser(ctx, freezeTime(tokWasValid))
	require.NoError(t, err)

	wf, err := tp.Parse(ctx, tok)
	require.NoError(t, err)

	assert.Equal(t, "push", wf.EventName)
	assert.Equal(t, "thepwagner/github-token-action", wf.Repository)
	assert.Equal(t, "thepwagner", wf.Actor)
	assert.Equal(t, "refs/heads/main", wf.Ref)
	assert.Equal(t, "Hack", wf.WorkflowName)
}

// I'd stop the world and verify tokens with you
func freezeTime(t time.Time) github.TokenParserOpt {
	return func(cfg *oidc.Config) {
		cfg.Now = func() time.Time { return t }
	}
}
